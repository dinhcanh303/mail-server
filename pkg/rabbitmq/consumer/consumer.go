...
// Configure implements EventConsumer.
...
// StartConsumer implements EventConsumer.
func (c *consumer) StartConsumer(fn worker) error {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	ch, err := c.createChannel()
	if err != nil {
		return errors.Wrap(err, "Create Channel Failed")
	}
...
func (c *consumer) createChannel() (*amqp.Channel, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Err ch.QueueDeclare")
	}
